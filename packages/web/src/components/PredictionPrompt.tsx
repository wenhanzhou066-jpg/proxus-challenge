  ...
  return (
    <section className="grid gap-2 rounded-xl border border-slate-800 bg-slate-900/60 p-4">
      <p className="font-semibold text-slate-200 text-sm">
        <span className="mr-1.5 rounded bg-amber-500/15 px-1.5 py-0.5 text-amber-300 text-xs uppercase tracking-wider">Predice</span>
        {prompt}
      </p>
      <div className="relative">
        <textarea
          ...
          rows={3}
          placeholder="Aventura una respuesta — vale cualquier cosa. Esto no puntúa."
      ...
      <div className="flex items-center justify-between">
        <span className="text-slate-500 text-xs">
          {ready ? "Listo" : `${minChars - text.trim().length} caracteres más`}
        </span>
        <button
          type="button"
          disabled={!ready}
